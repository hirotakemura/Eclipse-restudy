...
export function analyze(project: Project): Analysis {
  ...
  const strands: Strand[] = [
    ...
    },
    {
      id: "photos",
      /**
       * **区分の名前が違っていて、いちばん効く写真を一度も数えていなかった**（D-287）。
       *
       * `schema.ts` が決めている区分は「加工事例」なのに、ここは「加工品」を探していた。
       * **存在しない区分なので、必ず0点。** 台本には
       * 「加工したものが写っている写真が、いちばん問い合わせに繋がります」（docs/06）と
       * 書いてあるのに、**その写真だけが見立てに入っていなかった。**
       * 書き写した名前は、いつかずれる（D-197と同じ）。
       */
      score: (photoOf("工場・設備") ? 2 : 0) + (photoOf("外観") ? 1 : 0) + (photoOf("加工事例") ? 2 : 0),
      why: "現場の写真を預かっている",
    },
    {
      id: "people",
      /**
       * **写真は、どれを先に見せるかには効かない**（D-290。D-286と同じ理屈）。
       *
       * 代表者の写真に2点、働く人に1点を付けていたため、
       * **写真が届いた途端に代表の帯が先頭へ飛び出し、会社の話の順番が変わった**（実測）。
       * 「写真の有無でサイト全体のデザインが別物にならない」（ご指示§10）に反する。
       *
       * 代表の帯を出すかどうかは、**代表の言葉があるか**で決める。
       * 写真はその帯の中に出る（`People.astro`）。**それがメディアの層の役割である。**
       */
      score: text(p.executive?.vision) ? 2 : 0,
      why: "人を出せる材料がある",
    },
    {
      id: "history",
  ...
  if (isGeneral) {
    ...
    add("record", proven.length >= 2 ? 5 : proven.length >= 1 ? 3 : len(p.cases) ? 2 : 0,
      "結果まで数字で言える実績がある");
    /**
     * **写真があることは、強みの根拠ではない**（D-286）。
     *
     * 最初は代表者の写真に2点を付けていた。写真を1枚足しただけで
     * **最大の強みが `price` から `person` に変わり、手順書ごと入れ替わって
     * ページが別物になった**（検証用の工務店で実測）。
     * 「写真の有無でサイト全体のデザインが別物にならない」（ご指示§10）に正面から反する。
     *
     * そもそも**写真は誰でも撮れる。** 「人で選ばれている」の根拠は、
     * 代表の言葉・受け継いだ年月・沿革のほうである。
     * **写真はメディアの層であって、見立ての材料ではない**（`photos` の筋は別にある）。
     */
    add("person",
      (text(p.executive?.vision) ? 3 : 0)

...
export const BLOCKS: Block[] = [
  ...
  // ── E：制作条件（2分） ───────────────────────────────
  {
    ...
    fields: [
       ...
       * **見た目のご希望**（D-470）。
       *
       * 以前は末尾に「サイトの見た目」のタブを置き、型と9つの軸（配色・書体・雰囲気・メニュー・
       * 最初の画面・章の区切り・見出し・表・文字サイズ）を**取材の場でお客様と選んでいた**（D-137・D-167）。
       * だが、お客様が見るのは**小さな見本で、御社の中身が入ったサイトではない。**
       * 効いた見た目の判断（ロゴ・写真の大きさ・絵を背景にするか）は、どれも**書き出した実物を見て**
       * 初めて分かった。98万円のお客様はデザインを任せるためにお金を払っている。
       *
       * そこで**ここで伺うのは2つだけ**にした——
       *   ① どの見本が御社らしいか（型）… **決定ではなく、ご希望として**記録する
       *   ② 本文の文字の大きさ … 読み手（60代前後）の目の話なので、**社長ご本人に選んでいただく**（D-153）
       * 残りの軸は、書き出したサイトを見てこちらで決め、原稿のご確認のときに実物でお見せする。
       */
      {
        path: "theme",
        label: "見た目のご希望",
        customerLabel: "サイトの見た目のご希望",
        type: "theme",
        help:
          "画面を社長に向けて「どれが御社らしいですか」と伺う。**決定ではなく、ご希望として記録する。**" +
          "書き出したサイトを見てこちらで決め直し、原稿のご確認のときに実物でお見せする。" +
          "文字の大きさだけは、**社長ご本人の目で**選んでいただく（D-153）",
      },
      {
        path: "terms.colorRequest",
        label: "色のご希望",
        customerLabel: "色のご希望",
        type: "text",
        placeholder: "例：コーポレートカラーの紺は使いたい／赤は避けたい",
        help: "会社の色（ロゴ・看板・制服など）や、避けたい色があれば伺う。**配色はこちらで合わせる**",
      },
    ],
  },
];

...
## 第2段階の完了条件（アドバイザー確認・2026-09-15）

**第2段階はここで止める。** 製造業「量産・設備」「製品・開発」への `light` 拡張は行わない。

> 理由：現在の製造業には既に固有の視覚言語があり、現段階では装飾数を増やすことよりも、
> **「方向性ごとに何を視覚化するか」が異なって見えること**を優先するため。（アドバイザー）

### 確認した状態

| | 結果 |
|---|---|
| 汎用 モダン | **geometry**（幾何の線） |
| 汎用 力強い | **light**（淡い光）＋ geometry |
| 汎用 落ち着き・信頼 | **none** |
| `none` が完成形として成立するか | **○ 確認**（落ち着き・信頼は装飾0本で成立。紙の地・明朝・余白） |
| 写真なしでも未完成に見えないか | **○ 確認**（6型・写真0枚・PC/スマホ） |
| 新しいAsset語彙 | **追加していない**（`light` / `geometry` は第1段階の `AssetSubject` にある語） |
| レイアウト・カード | **変更していない** |
| モーション | **変更していない**（装飾に `animation` が1つも無いことを試験で確認） |
| library / generated | **実装していない**（`generated` は返す経路そのものが無い） |
| 製造業6型 | 装飾 **0本**（画素まで一致で確認） |

---

## Visual QA の原則（D-310・D-312 から）

**この2つは、以後どの段階でも外さない。**

### 原則A　CSSの宣言ではなく、重なり合った実際の描画結果を確認する（D-310より）

`npm run test:contrast` は **CSSの宣言**（`color:` と `background:`）を読んでいる。
**重なりの勝ち負けは見ていない。** だから

```
html[data-sections="alternate"] .section:nth-of-type(even)   0,3,1
.band[data-surface="dark"]                                   0,2,0
```

で縞が勝ち、**暗い地が薄い地に塗り替えられて白い文字だけが残った**（1.08:1）のを
3度も見落とした（D-232・D-237・D-310）。

**確かめるのは、ブラウザが実際に採用した値である。**
`npm run qa:assets` が、帯ごとに computed の地と文字色からコントラスト比を出す。

### 原則B　コントラストが正常でも、装飾が本文・数字・重要情報に重なっていないことを確認する（D-312より）

スマホで幾何の弧が本文を横切っていたとき、**コントラスト比は落ちていなかった。**
白い文字は弧の上にあるからである。**数字では捕まらない。**

**線を引く装飾は、文字の矩形と重なってはいけない。**
`npm run qa:assets` が、装飾の擬似要素の矩形と、本文・見出し・表・数字・札の矩形の
重なりを実測する。**地を塗る装飾（淡い光）は対象外**——あれは背景色であって、線ではない。

### 原則A・Bの実装（`npm run qa:assets`）

```
── 実際の描画結果を測る（原則A・B）── 42画面
  ○ 読めない帯なし／線の装飾が文字に重なっている画面なし
```
...
| 戻したもの | 出た内容 |
|---|---|
| D-310（縞の守り） | `✗ 汎用 力強い PC / 文字が読めません（declined）比 1.08` |
| D-312（スマホの位置） | `✗ 汎用 モダン スマホ / 線の装飾が文字に重なっています「施術ごとに薬剤と…」` |

**この検査自体も3回間違えた。** 測り方を直した記録：

1. 円を**外接する四角**で測っていた → 弧が通らない角の文字まで「重なった」（6件）
2. 文字を**要素の箱**で測っていた → 幅いっぱいの `<p>` の右端で重なった（4件）
3. `display:none` の擬似要素まで測っていた → **見えていない線**が重なった（1件）

いずれも「重なっている」と出したが、画面を見ると重なっていなかった。
**検査が赤いときも、まずコードではなく測り方を疑う**（D-309と同じ）。


---

## 付記：第3段階の実装（2026-09-15・D-315〜316）

**写真の要求（`wanted`）だけ。** 計算して持ち、data属性として出すところまで。

> 目的は「写真を増やす」ことではなく、
> **「どの情報を本物の写真で証拠化すると効果が高いか」をAsset層が判定できるようにすること。**（アドバイザー）

### いちばん大きい直し：**写真を実際に出している帯だけが、お客様の素材を名乗る**

第1〜2段階では「その置き場所に実写があるか」だけで `customer` を決めていた。
ところが**画面に画像を出すのは `photos` の帯だけ**である
（`CaseCard` `Equipment` `People` `Points` のどれにも `<img>` が無い）。

事例の表・設備のカード・会社概要の表は、**写真ではなく事実で証拠を出す帯**で、
その事例の写真は**すぐ隣の `photos` の帯**が引き受けている。

直す前は、**画像を1枚も描かない帯が「お客様の写真あり」と名乗っていた。**
「写真があるから使う」ではない（ご指示6）。**出している帯だけが名乗る。**

同じ理由で、**依頼もその写真を出す帯からだけ出す。**

```
実案件（松原精機・第1回取材まで）の写真の依頼

  直す前   25本   加工事例15 / 工場・設備3 / 代表者3 / 外観2 / 働く人2
  直した後  9本
...
**同じ「加工品の写真をください」が15回並ぶ紙は、人に渡して役に立たない。**
出す場所ごとに1本にすれば、**どの事例が欠けているか**が分かる。

### 優先度の決め方

**思いつきで決めない。** `analysis.ts` が写真の見立てに使っている点数をそのまま使う
（同じことを2箇所で決めないため・D-197）。
...
見立てに入っていない代表者・働く人は、証拠として要るが順位は下なので `medium`。
そのうえで、**山になる帯は一段上げ、控えめに置くと決めた帯は一段下げる。**

### 理由の文（`why`）

`wanted` が持つのは **`category` / `why` / `priority` の3つまで**（ご指示4）。
枚数は別の欄にせず、**人が読む理由の中に入れる。**

```
加工したものが写っている写真が、いちばん問い合わせに繋がります（4件のうち、まだ1枚も届いていません）
```

### 写真 0枚 / 少数 / 十分（A/B/C）

```
A 写真0枚      8ページ  写真の帯  0本  写真の依頼  4本  トップの山 declined:statement
B 少数（2枚）   8ページ  写真の帯  3本  写真の依頼  2本  トップの山 declined:statement
C 十分（7枚）   8ページ  写真の帯  5本  写真の依頼  0本  トップの山 declined:statement

骨格（写真の帯を除いた、内容と見せ方の並び）　○ A/B/C で同じ
写真が届くほど、依頼が減る（4 → 2 → 0）　　　○
写真が届くほど、お客様の写真を出す帯が増える（0 → 3 → 5）　○
```

面・余白・組み方は取り直される（帯が1本増えればリズムの計算が変わる）。
**骨格の定義は `qa:visual` と同じもの**を使う——同じものを2通りに定義しない（D-197）。

### 検査の結果

| 見るもの | 結果 |
|---|---|
| `npm test` | 631 → **645／全通過**（第3段階で14件・古い2件を第3段階の考え方に直した） |
| `npm run typecheck` / `astro build` | 通過・13ページ書き出し |
| `npm run qa:assets` | A/B/C・原則A・原則B すべて ○ |
| `npm run qa:companies` | 12ページのうち6ページが「帯は同じ・中身だけ違う」→ 基準を更新 |
| `npm run qa:visual` | 既存の数値がすべて同じ |
| **HTMLの差分** | 素材の属性（`data-asset-*`）を取り除くと、**変更前と1バイトも違わない（48/48枚）** |
| **画面の差分** | 6社 × 10ページ × PC/スマホ ＝ **90枚が画素まで完全一致** |

### 変えていないもの（ご指示8・9・10）

`visual.ts` `sections.ts` `analysis.ts` `theme.ts` `schema.ts`
`surface.ts` `motif.ts` `typography.ts` `motion.ts` `density.ts` `layout.ts` `peak.ts`
`site.css` `Present.astro` `Gallery.astro` `Figure.astro` ── **すべて無傷**

`library` / `generated` は実装していない。`brief` は引数に取っていない。

---

## 付記：第4段階の実装（2026-09-15・D-317〜319）

**Visual QA に A/B/C を正式に入れ、機械検査と人の目を分けた。** Asset Library には進んでいない。

### 機械が確かめること（`npm run qa:assets`）

**どれも「あってはいけないことが無い」だけを見る。** 数の多い少ないは見ない。

| | 内容 |
|---|---|
| 1 | 仮の画像（SVG）が残っていない |
| 2 | 中身の無い帯（見出しだけの帯）が無い |
| 3 | 画像の参照が切れていない |
| 4 | `customer` と言っている帯に本当に画像がある／画像を出しているのに名乗っていない帯が無い |
| 5 | 証拠の帯に graphic・library・generated が入らない（`npm test`） |
| 6 | A/B/C で骨格が変わらない |
| 7 | 線の装飾が本文・見出し・表・数字・札に重ならない（原則B） |
| 8 | コントラスト（重なり合った実際の描画結果・原則A） |
| 9 | 横にはみ出さない（PC・スマホ） |

**94画面を測って、すべて ○。**

### 人が見て決めること（`検査票.md` を書き出す）

**機械に言えるのはここまで。** 「写真0枚でも完成して見えるか」は人が画面を見て決める。
`npm run qa:assets` が7つの問いと、見るべき画面の一覧を書いた紙を出す。
...
**Asset数・graphic数・photo数は、どれも検査の行に作らない**（D-318）。
数にした瞬間に、その数を動かすことが目的になる。

### A/B/C

```
A 写真0枚      8ページ  写真の帯 0本  写真の依頼 5本  トップの山 declined:statement
B 少数（2枚）   8ページ  写真の帯 3本  写真の依頼 3本  トップの山 declined:statement
C 十分（7枚）   8ページ  写真の帯 5本  写真の依頼 0本  トップの山 declined:statement

骨格（写真の帯を除いた、内容と見せ方の並び）　○ A/B/C で同じ
写真が届くほど、依頼が減る　　　　　　　　　○ 5 → 3 → 0（置き場所ごとに1件）
写真が届くほど、写真を出す帯が増える　　　　○ 0 → 3 → 5
```

**数値を理想値として固定しない。** 見ているのは「増えたか」ではなく、
**骨格が動いていないか／依頼が正しく減るか／証拠が代替されていないか**である。

### 第4段階で見つけて直した不具合

**写真0枚の会社に、見出しだけの帯が出ていた**（D-317）。
`Gallery` は0枚なら何も描かないので、「外観」という見出しだけが残っていた。

帯ごと消したところ、**今度は写真が1枚も無いページから依頼が出なくなった**
（事例3件目・4件目の依頼が消えた）。いちばん頼みたいページから頼めない。

→ **判断としては残し、描画では落とす。**
「このページに何が要るか」と「このページに何を描くか」は別のことである。

さらに、`willDraw` を**枚数の合計**で見ていたため、外観と事例を2枚預かっている会社で
**「工場・設備」の見出しだけの帯**が出た。**その帯が出す置き場所**を見るように直した。

### `npm run gaps` への接続（D-319）

**計算は Asset層のまま**（`photoRequestsFor`）。gaps は並べ替えて出すだけ。
...
**1. 工場・設備**
- どこで使う：設備一覧
- なぜ　　　：設備の現物は、受けられるかどうかの判断に直結します（4件のうち、まだ1枚も届いていません）

**2. 加工事例**
- どこで使う：加工事例の一覧・1件目「他社が反りで断った薄肉カバー部品」・2件目「…」・3件目「…」・4件目「…」
- なぜ　　　：加工したものが写っている写真が、いちばん問い合わせに繋がります

## あると良い　2件 ／ ## 余裕があれば　1件
```

**置き場所ごとに1行にまとめ、どの事例のものかは残す。**
同じ頼みが並ぶ紙は役に立たないが、**違う事例の写真を1つに潰してもいけない。**

### 検査の結果

| 見るもの | 結果 |
|---|---|
| `npm test` | 645 → **653／全通過** |
| `npm run typecheck` / `astro build` | 通過・13ページ書き出し |
| `npm run qa:assets` | 機械検査9項目 **94画面すべて ○**／A/B/C すべて ○ |
| `npm run qa:companies` | 3ページが「空の写真の帯が消えた」→ 基準を更新 |
| `npm run qa:visual` | すべて基準を満たす |
| `npm run gaps` | 写真の章が出る（実案件で5件） |


---

## 付記：Visual Peak の修正（2026-09-15・D-320〜321）

**人が画面を見て判断した結果**（第4段階 Human QA）。

### 山は「見出しを大きくする仕組み」ではない

`data-role` が当たるのは**帯の `h2` だけ**である。山ごとに「何を大きくしているか」を並べると、
2つが**取り違えていた。**

| 山 | 主役であるべきもの | 中身側の拡大 | 直す前の見出し | 直したあと |
|---|---|---|---|---|
| number | 数字 | ○ | 中 | 中 |
| statement | 一言 | ○ | 中 | 中 |
| spec | 条件表 | ○ | 中（D-284で意図的） | 中 |
| **process** | **工程そのもの** | **○ 番号36px・工程見出し** | **最大** | **中** |
| **image** | **写真そのもの** | **✗ 何も大きくしていなかった** | **最大** | **中＋写真を1列16:9** |

実測（修正前）：
- 事例ページ「ご相談から結果まで」**約110px** ／ 工程見出し約20px ／ 本文16px
- 設備一覧「工場・設備」**約110px** ／ その下の写真は通常寸法

**会社固有の情報を1文字も持たない汎用の語が、読ませたい中身より強かった。**

### 直したもの（最小）

```
peak.ts   process.role: "display" → "sectionTitle"
          image.role:   "display" → "sectionTitle"
site.css  .band[data-peak="image"] .gallery   { grid-template-columns: 1fr; }
          .band[data-peak="image"] .photo img { aspect-ratio: 16 / 9; }
```

**構成・順番・presentation・語彙・Asset層・装飾は1つも動かしていない。**
`object-fit: cover` とスマホの畳み方は既定のまま。

### 結果

| 画面 | 修正前の山 | 修正後の山 |
|---|---|---|
| A2（写真0枚・事例1件目） | 大きな工程見出し | **工程そのもの**（番号・工程見出し・本文の階段） |
| B1（写真2枚・事例1件目） | 大きな工程見出し | **工程そのもの**（写真は証拠として上に） |
| C1（写真7枚・事例1件目） | 大きな工程見出し | **工程そのもの**（同上） |
| C2（設備一覧・写真あり） | 大きな設備見出し | **写真そのもの**（1列・16:9） |

画素で突き合わせた結果、**変化したのは狙った8枚だけ。** 製造業3方向・汎用3方向は無傷。

---

## 付記：第5段階の実装（2026-09-15・D-322〜326）

**作ったのは仕組みだけである。** 画像生成も、検索も、管理画面も、CMSも、アップロードも、
一括登録も、AIによる選定も、自動配置も、乱数も、**1つも作っていない。**

### 何を作ったか

| ファイル | 中身 |
|---|---|
| `lib/design/system/library.ts` | 素材の語彙・登録簿・登録時の検査・検索 |
| `lib/design/assets.ts` の `adoptLibrary()` | 採用の判断（`decorate()` の**あと**に走る） |
| `site-template/public/library/*.svg` | 最小セット2枚（KOBO内製） |
| `site-template/src/components/Band.astro` | `data-asset-library` と `--asset-image` を出す |
| `site-template/src/styles/site.css` | 地の層として敷く規則 |
| `asset.test.mjs` | 34件を追加（101件通過） |

### 1. 登録してよい主題を `texture` 1つに絞った（D-322）

ここがこの段階でいちばん重要な判断である。

ご指示の例には「工場らしい質感」「金属の質感」なども挙がっていた。
しかし**それらは画面では「その会社の実物」として読まれる。**
「イメージです」と添えても、**お客様の証拠として働いてしまう。**
ご指示の原則③④と正面から衝突するので、**そもそも登録できないようにした。**

```ts
export const LIBRARY_SUBJECTS = ["texture"] as const;
```

`workpiece` `facility` `exterior` `person` `workplace` `product` を登録しようとすると、
`assertLibrary()` が**読み込みの時点で落とす**（＝サイトが書き出せない）。
`light` `geometry` `grid` `dimension` は**CSSがすでに描けている**ので、来る理由がない。
**CSSの繰り返しでは作れないのは、紙と布の「不規則な目」だけ**である。

### 2. 証拠に届かないことを、型で固定した

```ts
export interface LibraryAsset {
  source: "library";      // リテラル固定
  intent: "atmosphere";   // リテラル固定
  ...
}
```

`intent` が `"atmosphere"` 以外を取れないので、
**「ライブラリ画像を実績写真として出す」コードは、書こうとしても型が通らない。**
D-206（内容と見せ方を型で分ける）と同じ思想で、注意書きではなく構造で止める。

### 3. 採用の順序を、コードの順序にした（D-324）

```
構成（composeVisual → composeAssets）
  → その構成が要求している主題（帯の subject と 型の assets）
  → graphic で満たせるか（decorate() を先に走らせてある）
  → ライブラリに候補があるか（findLibraryAsset）
  → 採用条件（素材側の usage.surfaces）
  → どれか1つでも欠ければ none のまま
```

「graphic で満たせるなら library に行かない」を、**判断の中の条件ではなく、順序で保証する。**
`adoptLibrary()` は `decorate()` の**あと**に走り、
**雰囲気の素材がすでに1つでもあるページには、何もしない。**

置く先は**山か主役の帯だけ**である。`decorate()` が持っている「それ以外の帯」への
逃げ道を、ここには作っていない。**控えめに置くと決めた帯に素材を敷くのは、
決めた控えめさを素材で打ち消しているということ**であって、品質は上がらない。

### 4. 地の層にしか敷かない（D-323）

`<img>` にすると、**画面でも、HTMLを読んでも、お客様の実績写真と見分けがつかない。**
`::before` の背景として敷けば、**取り違えようがない。**
場所は `--asset-image`（`Band.astro` がインラインで渡す）で、
**CSSにIDと場所の対応表は持たない**——単一の正は登録簿だけにする（D-197）。

### 5. 権利（D-326）

| 欄 | paper-grain | linen-grain |
|---|---|---|
| holder | KOBO | KOBO |
| terms | 自社制作・自社保有。商用可・改変可・クレジット不要 | 同左 |
| origin | KOBO内製（SVG・feTurbulence） | KOBO内製（SVG） |
| checked | 2026-09-15 社長 | 2026-09-15 社長 |

**1つでも空なら読み込みの時点で落ちる。**「あとで確認する」は登録の時点で通らない。
最小セットを2枚にしたのは、**増やすことを目的にしないため**である。

なお、`soft-light.svg` も一度作ったが**登録せずに消した。**
淡い光はCSSが描けているので、**登録すると「素材があるから使う」の入り口になる。**

### 6. 画面を見て直したこと（D-327）

`linen-grain.svg` は、最初は 6px の縦横の線で作った。
**検査は全部緑で、コントラストも 14.7:1 あった。**
しかし画面に出してみたら、**布ではなく方眼紙だった。**

方眼は `motif="grid"` が持っている別の語である。
**同じものを2つの名前で描く**ことになり、第2段階で `light` / `geometry` の名前を
地紋と衝突させないようにしたのと同じ問題を、素材の側で作っていた。

横方向に伸ばした乱れ（`baseFrequency="0.04 1.1"`）に替えて、繊維の向きが出る形にした。
**D-192「数字の検査は必要だが十分ではない」の、また一つの実例である。**

### 7. 実際に敷いたときのコントラスト（実測）

白い地の上に敷いて、素材の最も暗い画素で測った。

| 素材 | 地の最暗 | 本文とのコントラスト（最悪） | 素材なし |
|---|---|---|---|
| paper-grain | 242/255 | **14.22:1** | 15.92:1 |
| linen-grain | 246/255 | **14.74:1** | 15.92:1 |

D-310（striping が白抜きの帯を 1.08:1 まで落としていた）の再発を避けるため、
**面の条件（`usage.surfaces`）で白抜きの地を外し、実際に敷いた画素でも測った。**

### 8. `generated` への道は作っていない（D-326）

語彙には残っているが、**そこへ値を入れる関数はどこにも書いていない。**
「書いていない」は放っておくと崩れるので、`lib/` 全体を走査して
`source = "generated"` の代入が無いことを試験にした。

### 結果：採用0件（D-325）

松原精機・検証用6社・**全15型・全13ページ**で、ライブラリは**1枚も採られなかった。**

理由ははっきりしている。**紙の地（`surface=paper`）と地紋（`motif=grain`）が、
すでに `texture` を担っている。** つまり「graphic で満たせる」が全ページで成り立っていて、
ライブラリの出番がない。

**これは失敗ではない。** ご指示のとおり、成功とは「画像が増えた」ではなく
**「Libraryという選択肢が存在しても、必要なページだけが使い、不要なページでは `none` のまま成立する」**
ことである。いまは前者が0件で、後者が全ページである。

ただし**0件は、仕組みが壊れていても0件になる。**
だから「実案件では0件」と「条件を揃えれば採られる」を**両方**試験している。

| 確かめたこと | 結果 |
|---|---|
| 書き出したHTML（CSSを除く） | **13/13ページ バイト単位で一致** |
| 画面（PC・スマホ／13ページ） | **26/26 画素まで一致** |
| `npm test` | 全10本 緑（`asset.test.mjs` は 101/101） |
| `npm run typecheck` | 緑 |
| `npm run gaps`（松原精機） | 写真の依頼5件 変化なし |

### §11 の設計との差

第0段階で書いた `LibraryAsset`（§11）から、実装で変えたところがある。

| §11 | 実装 | なぜ |
|---|---|---|
| `subject: AssetSubject` | `subject: LibrarySubject`（`texture` のみ） | D-322。実物として読まれる主題を登録できなくするため |
| `plan` / `mood` / `aspectRatio` | 持たせていない | 使う場面が無い。**使わない欄は、いつかずれる**（D-197） |
| `license: { source, commercial, ... }` | `license: { holder, terms, origin, checked }` | 真偽値では「誰がいつ確かめたか」が残らない |
| （無し） | `usage: { as, maxPerPage, surfaces }` | **採用条件を素材の側に持たせる。** 判断する側で書き写さない |

### 残っている問題

- **仕組みが一度も動いていない。** 実案件で採用が出るのは、構成の側が「紙の地を使わない型」を
  持ったときである。それまでは、試験の中でしか動かない。
- ご指示の例にあった「工場らしい質感」は**採用しなかった**（D-322）。
  これは指示への不服従ではなく、**同じご指示の原則③④を守るとそうなる**という報告である。
  実物として読まれる主題を入れるなら、**画面に「これは素材です」と示す仕組みが先に要る。**

---

## 付記：第5段階の追加検証（2026-09-15・D-328〜330）

アドバイザーのご指示で、**「必要なときだけLibraryが自然に選ばれる」**が本当に成立しているかを検証した。

### 直したこと：採用条件を「ページ単位」から「帯の主題単位」へ（D-330）

初版は**ページに雰囲気の素材が1つでもあれば足さない**としていた。
その結果、**15型・945ページのどこでも採用0件**になった。

しかし要求は帯ごとに立つ。沿革（`history`）と素材（`materials`）は、
**内容そのものが「素材の目」を求めている**（`SUBJECT_OF` が `texture` を返す）。
だから**満たせるかどうかも帯ごとに見る**のが正しい。

```
その帯が求めている主題を、CSSが描けるか
  light / geometry → 描ける（第2段階）→ ライブラリに来ない
  grid / dimension → 地紋がある        → ライブラリに来ない
  texture          → **繰り返しでは作れない** → ここだけが来る
```

そのうえで、**すでに雰囲気の素材がある帯から2本以上離す**——
第2段階で light と geometry に課したのと同じ規則を使い回している。

### 検証したケース

**`product`（製品・開発）の型の、会社概要ページの「沿革」の帯。**

無理な条件を作っていない。この型は `assets` に `texture` を挙げているのに、
**構成が紙の地（`surface="paper"`）を使わない唯一の型**である。
沿革の帯は無地の上に置かれ、地紋も無い。**CSSでは出せない紙の目が、そこで初めて要る。**

| ご指示の確認項目 | 結果 |
|---|---|
| ① library候補が要求される | `history` の帯（`subject=texture`・`source=none`・`motif=none`・`surface=plain`） |
| ② 素材があれば採用される | `paper-grain` が採用。HTMLに `data-asset-library="paper-grain"` が出る |
| ③ 素材が無ければ `none` | **登録簿を空にして同じ構成を組み直し**、帯の数・内容・面・余白が1つも変わらないことを確認 |
| ④ graphicで十分なら採用しない | `craft`・`classic` は帯の地そのものが紙。**空いた帯が必ず紙の帯の隣になり、降りてこない** |
| ⑤ 顧客の証拠を置き換えない | 証拠の帯は `intent=evidence` なので、そもそも候補にならない（型で止まる） |
| ⑥ 枚数増加になっていない | **945ページ中13ページ（1.4%）。1ページ1枚。** 採られた帯は例外なく `materials` か `history` |
| ⑦ 採用前後のスクリーンショット | 下記 |

### 採用前後（`product` / 会社概要 / 沿革の帯）

- **前**：真っ白な地に見出しと箇条書きだけ
- **後**：帯全体にごく薄い紙の目。**帯の境目が分かるようになり、「沿革が紙の上に置かれている」ように読める**

**効果は弱い。** 拡大しないと分からない程度である。
ただしこれは**意図した強さ**でもある（本文のコントラストは 14.22:1 で、素材なしの 15.92:1 からほとんど落ちていない）。
**強くすれば良くなるものではない**ので、ここは画面を見ながら決める余地として残す。

### 見つけた重大な欠陥（D-329）

この検証で撮った画面に、**ライブラリとは無関係の重大な欠陥**が写っていた。

**白抜きの帯の中の表が、白い文字と白い地になっていた。18マス中14マスが 1.10〜1.16:1。**

D-310（章の区切りの縞が暗い帯を塗り潰していた）と**まったく同じ形**である。
あのとき縞だけを直して、**表を見ていなかった。**

- `test:contrast` は**宣言**を見るので通る
- `qa:assets` は撮る6型に `product` が入っていないので、**91画面の検査を緑のまますり抜けた**

直したあと、実測 8.10〜16.56:1、4.5:1 未満は0件。縞は残してある。
**松原精機の画面は26/26変わらず**（この会社の表は白抜きの帯に無い）。

### 測り方の誤り（9度目・10度目）

- 半透明の地（`rgba(255,255,255,.16)`）を**そのまま白として読み**、直した表を「1:1」と報告した。**下の地に重ねてから測る**のが正しい
- 書き出し先を `site-draft` だけ見ていた。**公開判定を通った案件は `site` に出る**ので「書き出せず」と誤判定した

どちらもコードではなく測り方が壊れていた。**検査が赤いときも、まず測り方を疑う。**
